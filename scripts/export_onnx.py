...
def assemble():
    ...
    print(f"quantizing {onnx_src.name} -> {q4.name}", file=sys.stderr)
    quant = MatMulNBitsQuantizer(str(onnx_src), bits=4, block_size=32, is_symmetric=True)
    quant.process()
...
def patch_js_config(path):
    ...
    cfg["transformers.js_config"] = {"dtype": "q4"}
